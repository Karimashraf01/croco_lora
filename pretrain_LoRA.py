...
def get_args_parser():
    ...
    parser.add_argument("--experiment_name", default="LoRA_on_ENCODER_Decoder_r4_qkv_proj", type=str, help="Name of the experiment for logging and checkpointing")
    # dataset 
    parser.add_argument('--training_pairs', default='croco_pairs_train.txt', type=str, help="training paris txt file")
    parser.add_argument('--Validation_pairs', default='croco_pairs_val.txt', type=str, help="validation pairs txt file")
    # training 
    ...
    parser.add_argument('--lora_qkv_only', type=int, default=0, choices=[0,1])
    parser.add_argument('--lora_enc_only', type=int, default=0, choices=[0,1])
    return parser
...
def main(args):
    args.lora_qkv_only = bool(args.lora_qkv_only)
    args.lora_enc_only = bool(args.lora_enc_only)
    ...
    # 2️⃣ Inject LoRA EXACTLY as training
    model._lora_on_enc_blocks(r=args.lora_r, alpha=args.lora_alpha, dropout=args.lora_dropout,qkv_only=args.lora_qkv_only)
    if not args.lora_enc_only:
        model._lora_on_dec_blocks(r=args.lora_r, alpha=args.lora_alpha, dropout=args.lora_dropout,qkv_only=args.lora_qkv_only)


    # 2️⃣ Freeze everything
    for param in model.parameters():
        param.requires_grad = False

    # 3️⃣ Unfreeze LoRA params only
    model._unfreeze_LoRA_parameters()
    # print(type(model.enc_blocks[0].attn.qkv))
    # exit()


    # 3️⃣ Load base weights
    if "base_state_dict" in ckpt:
        model.load_state_dict(ckpt["base_state_dict"], strict=False)

    # 4️⃣ Load LoRA weights
    if "lora_state_dict" in ckpt:
        model.load_state_dict(ckpt["lora_state_dict"], strict=False)
    args.resume = None  # avoid loading again in misc.load_model later
